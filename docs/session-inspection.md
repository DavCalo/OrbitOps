...
Start with the supported synthetic sample from the repository root:
...
than being silently removed from the report. Examples later in this document that use
`sessions/mission-*.jsonl` assume those files were generated by the README manual mission pass (or
that you substitute paths to evidence files you already have); they are not included in a fresh
clone.

## Reading the report

For a first pass through the text report, read the sections in this order:

1. `status` and `evidence` — whether the selected evidence is usable and the limits of what the
   inspector can claim;
2. `SUMMARY` and `SOURCES` — which evidence lanes are present and their whole-session counters;
3. `DIAGNOSTICS` — gaps, corruption, incompatibility, or truncation that needs attention;
4. `TIMELINE` — detailed source-local events to inspect after a diagnostic or counter identifies an
   area of interest.

The phrase `operator-selected bundle; cross-stream provenance unverified` is deliberately
conservative. It means each selected stream was validated under its own format contract, while the
inspector does not cryptographically prove that independently selected files originated from one
common run. A fingerprint is a SHA-256 identifier of behavior-affecting configuration or policy
values used for reproducibility. It is not personal or biometric information, authentication, a
signature, or a provenance proof.
...
The demo is terminal-only and does not launch a graphical application. The README visual is a
static representation generated from a real captured run rather than a fabricated mock-up.
Regenerate it after intentional demo-output changes with:
